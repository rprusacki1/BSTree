...
#include "d_stiter.h"
	...
	void displayTree();
	//In order display
	void inOrder(stnode<T> *t, const string& seperator = " ");
...
template <typename T>
void stree<T>::displayTree() {
	stnode<T> *t = root; //start print at root
	inOrder(t, " ");
	cout << endl;
}

template <typename T>
void stree<T>::inOrder(stnode<T> *t, const string& seperator) {
	if (t != NULL) {
		inOrder(t->left, seperator); //left
		cout << t->nodeValue << seperator; //root
		inOrder(t->right, seperator); //right
	}
}
